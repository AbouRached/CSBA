...
def test_department_user_sees_only_matching_branches(env):
    p = Principal(user_id=102, username="alpha_support", role="department", customer_id=1, department_ids=[10])
    with env["db"].conn() as c:
...
def test_did_branch(env):
    p = Principal(user_id=104, username="fresh", role="department", customer_id=1, department_ids=[11])
    with env["db"].conn() as c:
        where, params = scope_sql(c, p, None)
...
def test_customer_admin_sees_whole_drive_only(env):
    p = Principal(user_id=101, username="alpha_admin", role="customer_admin", customer_id=1)
    with env["db"].conn() as c:
        where, params = scope_sql(c, p, 2)  # asks for Beta, must be ignored
    names = _rows(env, where, params)
